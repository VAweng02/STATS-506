...
hist(gasoline_cars$fuel_city_mpg, 
     breaks = 20, 
     main = "Distribution of highway gas mileage", 
     xlab = "highway gas mileage", 
     col = "blue")

# create new variable "log_fuel_city_mpg" for log-transformed fuel_city_mpg
gasoline_cars$log_fuel_city_mpg <- log(gasoline_cars$fuel_city_mpg)

# histogram of log-transformed gas mileage
hist(gasoline_cars$log_fuel_city_mpg, 
     breaks = 20, 
     main="Log-Transformed Highway Gas Mileage", 
     xlab="Log(Gas Mileage)",
     col = "blue")
```
...
```{r}
model <- lm(log_fuel_city_mpg ~ eng_torque + eng_horsepower + dim_length + dim_width + dim_height + as.factor(id_year), data = gasoline_cars)
summary(model)
```
